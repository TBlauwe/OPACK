...
namespace opack
{
	...
	flecs::entity behaviour(World& world, TFunc&& func)
	{
		...
		world.system<TInputs ...>(fmt::format("BehaviourActivation_{}", friendly_type_name<TBeh>().c_str()).c_str())
			.term(flecs::IsA).template second<const opack::Agent>()
			.template term<HasBehaviour, TBeh>().read_write()
	...
	*/
	...
	public:
		FlowBuilder(World& world) : 
			world{ world },
			flow_system { world.system(fmt::format("Launching_{}", friendly_type_name<T>().c_str()).c_str()) }
		{
		    ...
		    world.system(fmt::format("Cleaning_{}", friendly_type_name<T>().c_str()).c_str())
	...
	public:
		...
		OperationBuilder(World& world) : 
			world{ world },
			operation {world.entity<TOper>(friendly_type_name<TOper>().c_str())},
			system_builder {world.system<TInput...>(fmt::format("System_{}", friendly_type_name<TOper>().c_str()).c_str())}
		{
